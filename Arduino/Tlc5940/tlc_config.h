...
/** Number of (daisy-chained) TLCs to run in parallel with separate SIN ports on the Arduino
	For NUM_TLCS_P > 1, you must run in Bit-bang mode (DATA_TRANSFER_MODE = TLC_BITBANG)
	ex1: NUM_TLCS=3, NUM_TLCS_P=1
	Arduino -> TLC -> TLC -> TLC
	
	ex2: NUM_TLCS=2, NUM_TLCS_P=4
	Arduino -> TLC -> TLC
		|-	-> TLC -> TLC
		|-	-> TLC -> TLC
	
	ex3: NUM_TLCS=1, NUM_TLCS_P=2
	Arduino -> TLC
		|-	-> TLC
	*/
#define NUM_TLCS_P    3
...
/* --------------------------------------------------------------------------- ADDED For Sequencer (START) */
/** SIN1 - 2nd parallel output from Arduino (Mega pin 49)*/
#define SIN1_PIN        PL0
#define SIN1_PORT       PORTL
#define SIN1_DDR        DDRL
/** SIN2 - 3rd parallel output from Arduino (Mega pin 48) */
#define SIN2_PIN        PL1
#define SIN2_PORT       PORTL
#define SIN2_DDR        DDRL
/** add more SINx_ on unused digital ports, if required, up to x = NUM_TLCS_P-1  */
/* --------------------------------------------------------------------------- ADDED For Sequencer (STOP) */

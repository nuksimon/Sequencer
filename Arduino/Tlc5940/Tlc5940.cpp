...
/* modified by Simon Nuk
	Jan 2013
	Added support for running TLCs in Parallel
	- tlc_shift8_p_init	(NOTE: still hardcoded for 3 parallel ciruits.  add elegant parameter solution later)
	- tlc_shift8_p		(NOTE: still hardcoded for 3 parallel ciruits.  add elegant parameter solution later)
	- update_p			(NOTE: still hardcoded for 3 parallel ciruits.  add elegant parameter solution later)
	- set_p
*/
...
uint16_t tlc_GSData_arr[NUM_TLCS_P][NUM_TLCS*16];	// --------------------------------------------------------------------------- ADDED For Sequencer 		
...
void Tlc5940::init(uint16_t initialValue)
{
	...
	tlc_shift_p_init();  // --------------------------------------------------------------------------- ADDED For Sequencer  
#endif
...
/* --------------------------------------------------------------------------- ADDED For Sequencer (START) */
/** Update function for Parallel mode */


uint8_t Tlc5940::update_p(void)
{
    if (tlc_needXLAT) {
        return 1;
    }
	...
	for (int i_tlc = NUM_TLCS*16-1; i_tlc >= 0; i_tlc--){				 
        tlc_shift12_p(tlc_GSData_arr[0][i_tlc], tlc_GSData_arr[1][i_tlc], tlc_GSData_arr[2][i_tlc]);       		
	}
...
/** Set function for Parallel mode 
	\param parallel (0 to #NUM_TLCS_P-1).  Indicates the parallel TLC circuit branch to use
*/
void Tlc5940::set_p(TLC_CHANNEL_TYPE channel, uint16_t value, uint8_t parallel)
{
    tlc_GSData_arr[parallel][channel] = value;
}

/* --------------------------------------------------------------------------- ADDED For Sequencer (STOP) */
...
void Tlc5940::setAll(uint16_t value)
{
    ...
    uint8_t *p = tlc_GSData;
    while (p < tlc_GSData + NUM_TLCS * NUM_TLCS_P * 24) { // --------------------------------------------------------------------------- CHANGED For Sequencer 
        *p++ = firstByte;
        *p++ = secondByte;
        *p++ = (uint8_t)value;
    }
...
/* --------------------------------------------------------------------------- ADDED For Sequencer (START) */
/** Sets all the bit-bang pins to output */
void tlc_shift_p_init(void)
{
    SIN_DDR |= _BV(SIN_PIN);   // SIN as output
	SIN1_DDR |= _BV(SIN1_PIN);   // SIN1 as output
	SIN2_DDR |= _BV(SIN2_PIN);   // SIN1 as output
    SCLK_DDR |= _BV(SCLK_PIN); // SCLK as output
    SCLK_PORT &= ~_BV(SCLK_PIN);
}

/** Shifts a byte out, MSB first */
void tlc_shift8_p(uint8_t byte, uint8_t byte1, uint8_t byte2)
{
    for (uint8_t bit = 0x80; bit; bit >>= 1) {
        if (bit & byte) {
            SIN_PORT |= _BV(SIN_PIN);
		...
		if (bit & byte1) {
            SIN1_PORT |= _BV(SIN1_PIN);
        } else {
			SIN1_PORT &= ~_BV(SIN1_PIN);
        }
		if (bit & byte2) {
			SIN2_PORT |= _BV(SIN2_PIN);
        } else {
			SIN2_PORT &= ~_BV(SIN2_PIN);
        }
        pulse_pin(SCLK_PORT, SCLK_PIN);
...
/** Shifts 12 bits out, MSB first */
void tlc_shift12_p(uint16_t val0, uint16_t val1, uint16_t val2)
{
